...
포트폴리오용으로 정리하면서 화면을 하나씩 다시 눌러보다가 회원가입 쪽에서 꽤 황당한 버그를 하나 발견했습니다. 비밀번호와 주소 값을 가져오는 코드가 실제 input id가 아니라 password, address라는 엉뚱한 이름으로 요소를 찾고 있었습니다. 실제 id는 pw, ad였습니다. 그러다 보니 항상 null이 잡혔고, 그 값에 접근하려는 순간 에러가 나면서 검증 로직 자체가 실행되지 않고 있었습니다. 더 황당한 부분은 그 에러 때문에 바로 아래 있던 코드, 그러니까 검증을 통과하면 존재하지도 않는 페이지로 강제 이동시키던 코드까지 같이 실행이 안 됐다는 점입니다. 그 덕분에 폼이 그냥 원래대로 제출돼버렸고, 회원가입 자체는 의도와 상관없이 우연히 되고 있었습니다. id를 제대로 맞추고, 필요 없어진 강제 이동 코드는 지웠습니다.
