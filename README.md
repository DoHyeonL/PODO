...
로그인 화면은 폼 UI와 회원가입 페이지 이동 기능을 구현했습니다(아이디·비밀번호 찾기는 UI만 있고 아직 기능은 준비 중입니다). 메인 화면은 T-Map API를 활용하여 지도를 구현하고, 경찰서·소방서·CCTV·편의점 마커를 카테고리별로 토글하여 노출 여부를 제어할 수 있도록 구성했습니다. 경로탐색 화면은 출발지와 목적지를 입력하면 실제 T-Map 경로 API로 경로를 안내하고, 경로 주변 실제 안전시설 데이터로 안전 점수를 계산해서 보여주도록 구현했습니다. 보호자 화면은 등록된 보호자 목록을 조회하고, 실시간 위치를 확인해서 보호자에게 알림을 보낼 수 있도록 구현했습니다.
...
포트폴리오 정리 막바지에 전체 화면을 하나씩 다시 눌러보다가, 회원가입 검증 코드에서 꽤 황당한 버그를 발견했습니다. 비밀번호와 주소 입력값을 가져올 때 `getElementById('password')`, `getElementById('address')`로 찾고 있었는데, 실제 input의 id는 `pw`, `ad`였습니다. 그래서 항상 null을 찾은 다음 그 값에 접근하려다 매번 오류가 나면서 검증 로직 자체가 실행되지 않고 있었습니다. 문제는 그 오류 덕분에 바로 뒤에 있던, 검증 통과 시 존재하지도 않는 페이지로 강제 이동시키던 코드까지 같이 실행이 안 됐고, 그 바람에 브라우저가 원래 폼을 그대로 제출해버려서 회원가입 자체는 우연히 되고 있었다는 겁니다. 겉보기엔 잘 동작하는 것 같아도 그 이유가 의도한 게 아니라면 언제든 다시 깨질 수 있다는 걸 깨닫고, id를 바로잡고 불필요한 강제 이동 코드를 지워서 제대로 된 흐름으로 고쳤습니다.
