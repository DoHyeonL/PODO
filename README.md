...
로그인 화면은 폼과 아이디·비밀번호 찾기, 회원가입 이동 기능을 구현했습니다. 메인 화면은 T-Map API를 연동해 지도를 띄우고, 경찰서·소방서·CCTV·편의점 마커를 카테고리별로 토글해서 켜고 끌 수 있게 구성했습니다. 경로탐색 화면은 출발지와 목적지를 입력하면 추천 경로를 보여주는 레이아웃을 맡았고, 보호자 화면은 등록한 보호자 정보를 수정하거나 삭제할 수 있도록 구현했습니다.
