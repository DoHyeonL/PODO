...
회원가입 검증 코드에서도 버그를 하나 발견했습니다. 비밀번호와 주소 값을 가져올 때 실제 input id가 아니라 password, address라는 이름으로 요소를 찾고 있었습니다. 정작 진짜 id는 pw, ad였습니다. 그래서 항상 null이 잡혔고, 그 값을 쓰려는 순간 에러가 나면서 검증 로직 자체가 실행되지 않고 있었습니다. 그런데 그 에러 때문에 바로 아래 있던 코드, 검증을 통과하면 존재하지도 않는 페이지로 강제 이동시키던 코드까지 같이 실행이 안 됐습니다. 결국 폼이 그냥 원래대로 제출돼버렸고, 회원가입 자체는 의도와 상관없이 되고 있었던 겁니다. id를 맞는 값으로 고치고, 필요 없어진 강제 이동 코드는 지웠습니다.
